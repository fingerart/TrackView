...
public class AudioMeter extends Thread {
    ...
    private void createAudioRecord() {
        ...
        for (int sampleRate : new int[]{8000, 11025, 16000, 22050, 32000, 44100, 47250, 48000}) {//采样率
            for (short audioFormat : new short[]{AudioFormat.ENCODING_PCM_16BIT, AudioFormat.ENCODING_PCM_8BIT}) {//编码制式
                for (short channelConfig : new short[]{AudioFormat.CHANNEL_IN_MONO, AudioFormat.CHANNEL_IN_STEREO,
                        AudioFormat.CHANNEL_CONFIGURATION_MONO, AudioFormat.CHANNEL_CONFIGURATION_STEREO}) {//声道
